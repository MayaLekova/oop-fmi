...
class DynamicArray {
 public:
  ...
  template <typename P>
  friend std::ostream& operator<<(std::ostream& out, const DynamicArray<P>& array);
...
std::ostream& operator<<(std::ostream& out, const DynamicArray<T>& array) {
  for (size_t i = 0; i < array.size(); ++i) {
    // Version 1: uses the getter `element_at`
    // out << array.element_at(i) << " ";

    // Version 2: reads elements directly, so operator<< should be
    // mentioned as `friend` in the class declaration
...
template <typename T>
std::istream& operator>>(std::istream& in, DynamicArray<T>& array) {
  T tmp;
  while (!in.eof()) {
    in >> tmp;
    // Uses the accessor `add`, so operator>> doesn't need to be `friend`.
    array.add(tmp);
  }
...
int main() {
  ...
  DynamicArray<float> other_temps;
  std::cin >> other_temps;
